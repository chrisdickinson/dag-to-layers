...
function assignLayerCandidate(vertices, incoming, outgoing, W, c, getTo) {
  ...
  while (assigned.size !== vertices.size) {
    ...
    for (let vertex of vertices) {
      ...
      let vertexOutgoing = outgoing.get(vertex) || φ
      ...
      if (!okay && vertexOutgoing.size) continue

      if (selectedDegree < vertexOutgoing.size) {
        selectedDegree = vertexOutgoing.size
        selected = vertex
      }
    }
    ...
    if (selected) {
      ...
      widthUp = widthUp + (incoming.get(selected) || φ).size
    }

    if (!selected || (widthCurrent >= W && selectedDegree < 1) || widthUp >= W * c) {
